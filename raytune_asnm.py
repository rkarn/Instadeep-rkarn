...
def random_search(task_data, task_id=0):
    ...
    analysis = tune.run(
        ...
        sync_config=SyncConfig(upload_dir="s3://rkarn-28d6244ed4c54337-inputs", sync_to_cloud="aws s3 sync {source} {target} --only-show-errors --endpoint-url http://minio-gateway.ichor:9000"))
    time.sleep(1)
